...
def test(config_params, model, dataloader_test, batches_test, df_test, path_to_results_xlsx):
    """Testing"""
    model.eval()
    total_images=0
    test_loss = 0
    ...
    with torch.no_grad():
        for test_idx, test_batch, test_labels, views_names in dataloader_test:
            ...
            if config_params['viewsinclusion'] == 'all' and config_params['learningtype'] == 'MIL' and (config_params['dataset'] == 'mgm' or config_params['dataset'] == 'cbis-ddsm'):
    ...
    if config_params['viewsinclusion'] == 'all' and config_params['learningtype'] == 'MIL' and (config_params['dataset'] == 'mgm' or config_params['dataset'] == 'cbis-ddsm'):
        evaluation.write_results_viewwise(path_to_results_xlsx, 'metrics_view_wise', count_dic_viewwise)
