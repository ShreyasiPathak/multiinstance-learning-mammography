...
class CNBlock(nn.Module):
    def __init__(
        ...
        self.block_kernel_sizes.extend([7, 1, 1])
        self.block_strides.extend([1, 1, 1])
        self.block_paddings.extend([3, 0, 0])
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        result += input
        #print("from convnext model:", result.shape)
        return result
...
class ConvNeXt(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.kernel_sizes.extend([4])
        self.strides.extend([4])
        self.paddings.extend([0])

        total_stage_blocks = sum(3*cnf.num_layers for cnf in block_setting)
        self.total_layer = total_stage_blocks
        stage_block_id = 0
        for cnf in block_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                block_call = block(cnf.input_channels, layer_scale, sd_prob)
                stage.append(block_call)
                stage_block_id += 1
                block_kernel_sizes, block_strides, block_paddings = block_call.block_conv_info()
...
def convnext_tiny_26_features(pretrained=False, **kwargs):
    model = convnext_tiny(pretrained=pretrained, weights=ConvNeXt_Tiny_Weights.DEFAULT)
    with torch.no_grad(): 
