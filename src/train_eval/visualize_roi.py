...
from train_eval import test, mask_roi_match
...
def model_output(config_params, model, dataloader_test, df_test):
    ...
    with torch.no_grad():
        for test_idx, test_batch, test_labels, views_names in dataloader_test:
            ...
            save_dir = os.path.join(config_params['path_to_output'], "visualization", "malignant-to-malignant", "{0}.png".format(filename))
            #print(save_dir)
            ...
            if iou_any_roi_max!=[]:
                df[exam_name] = [exam_name, iou_any_roi_max, iou_all_roi_mean, iou_any_roi_max_highestattnwt]
            ...
            print("iou exam any roi:", iou_any_roi_max)
            print("iou exam any roi:", iou_all_roi_mean)
            print(test_labels, test_pred)
            #input('halt')
    df_img = pd.DataFrame.from_dict(df, orient='index', columns=['ImageName', 'iou_any_roi_max', 'iou_all_roi_mean', 'iou_any_roi_max_highestattnwt'])
    df_img.to_csv(os.path.join(config_params['path_to_output'], "iou_score_test_set.csv"), sep=';',na_rep='NULL',index=False)
    iou_avg_any_roi = iou_sum_any_roi/df_img.shape[0]
    iou_avg_all_roi = iou_sum_all_roi/df_img.shape[0]
    iou_avg_any_roi_hattnwt = iou_sum_any_roi_hattnwt/df_img.shape[0]
