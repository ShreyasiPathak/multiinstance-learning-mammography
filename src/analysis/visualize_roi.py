...
def seg_evaluation(config_params, input_img_case, img_path_list, patch_locations_case, patch_img_case, patch_attentions_case, img_attention_case, views_names, exam_name, df_score, seg_eval_metric):    
...
def modelpatch_roi_match(config_params, model, dataloader_test, df_test):
    ...
    with torch.no_grad():
        for test_idx, test_batch, test_labels, views_names in dataloader_test:
            ...
            df_iou = seg_evaluation(config_params, loaded_image, img_path_list, patch_locations, patch_imgs, patch_attentions, img_attns, views_names, exam_name, df_iou, 'IOU')
            df_dsc = seg_evaluation(config_params, loaded_image, img_path_list, patch_locations, patch_imgs, patch_attentions, img_attns, views_names, exam_name, df_dsc,'DSC')
            '''if iou_any_roi_max!=[]:
