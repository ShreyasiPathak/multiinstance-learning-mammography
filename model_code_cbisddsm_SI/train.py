...
def results_store_excel(train_res, val_res, test_res, per_model_metrics, correct_train,total_images_train,train_loss,correct_test,total_images_test,test_loss,epoch,conf_mat_train,conf_mat_test, lr):
    lines = [epoch+1, lr]
    if train_res:
    ...
    if val_res:
        ...
        lines.extend([avg_test_loss, accuracy_test, f1macro_test, recall_test, speci_test])
...
def train(model,data_iterator_train,data_iterator_test,batches_train,batches_test,epochs):
    ...
    for epoch in range(start_epoch,epochs):
        ...
        if patience_epoch:
            correct_test,total_images_test,loss_test,conf_mat_test = validation(model, data_iterator_test, epoch, batches_test, class_imbalance)
            valid_loss=loss_test/total_images_test
            results_store_excel(True, True, False, None, correct_train,total_images_train,loss_train,correct_test,total_images_test,loss_test,epoch, conf_mat_train, conf_mat_test, current_lr)
...
def validation(model, data_iterator_val, epoch, batches_val, class_imbalance1):
    ...
    return correct,total_images,val_loss,conf_mat_test#,loss_ar_val
...
if __name__=='__main__': #before calling train, create the config.ini file for this model run using the code config_file_creation.py
    ...
    resize=int(config_object["parametersetting"]['resize'])
    ...
    if run_descrip:
        path_to_model=path_to_output+"model_"+str(rand_seed)+'_'+run_descrip+".tar"
        path_to_results=path_to_output+"result_"+str(rand_seed)+'_'+run_descrip+".xlsx"
        path_to_results_text=path_to_output+"result_"+str(rand_seed)+'_'+run_descrip+".txt"
        path_to_learning_curve=path_to_output+"learningcurve_"+str(rand_seed)+'_'+run_descrip+".png"
        path_to_log_file=path_to_output+"log_"+str(rand_seed)+'_'+run_descrip+".txt"
    else:
    ...
    if os.path.isfile(path_to_results):
        ...
        header=['Epoch','lr','Avg Loss Train','Accuracy Train','F1macro Train','Recall Train','Speci Train','Avg Loss Val','Accuracy Val','F1macro Val','Recall Val','Speci Val']
    ...
    model = model.SILmodel(activation, featureextractormodel, extra, topkpatch)
    ...
    if datascaling=='scaling':
        if featureextractormodel:
            if 'resnet18pretrained' in featureextractormodel or 'resnet34pretrained' in featureextractormodel or 'resnet50pretrained' in featureextractormodel or 'densenet' in featureextractormodel:
    ...
    dataset_gen_train = utils.BreastCancerDataset_generator(df_train,modality, datascaling, resize, flipimage, inchans, preprocess_train)
    ...
    if patience_epoch:
        dataset_gen_val = utils.BreastCancerDataset_generator(df_val, modality, datascaling, resize, flipimage, inchans, preprocess_val)
    ...
    dataset_gen_test = utils.BreastCancerDataset_generator(df_test, modality, datascaling, resize, flipimage, inchans, preprocess_val)
    ...
    try:
        ...
        sheet3.append(['Loss','Precision','Recall','Specificity','F1','F1macro','F1wtmacro','Acc','Bal_Acc','Cohens Kappa','AUC'])
        per_model_metrics, conf_mat_test = test.test(model1, dataloader_test, batches_test, activation, sheet2, sheet3, sheet4, device, classes, df_test)
